...
void RosThread::run()
{
    ...
    pressure_sub_ = m_nodeHandler->subscribe<stm_client::atm_arr>("/pressure_status", 1, &RosThread::pressureCallback, this);
...
/*!
 * \brief RosThread::pressureCallback gets values from pressure sensors and sends to UI
 * \param msg
 */
void RosThread::pressureCallback(const stm_client::atm_arr::ConstPtr &msg){

    for(int i =0; i<3; i++)
    {
       pres_val[i]= msg->data[i]*10000;

}

    emit pressureCallback(pres_val);
